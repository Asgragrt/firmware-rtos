    ...
    switch ( led_array->mode ) {
        case _double_wave:
            if ( current_led < LED_COUNT / 2 + LED_COUNT % 2 ) {
                dif = abs_m( ( int16_t ) led_array->led_counter -
                             ( int16_t ) current_led );
            } else {
                dif = abs_m( LED_COUNT - 1 - led_array->led_counter -
                             current_led );
            }
...
void wave_duty( led_array_t* led_array, uint8_t current_led,
                uint16_t* led_duty ) {
    uint16_t led_pos = wave_led_pos( led_array, current_led );
    uint16_t target_duty =
        ( uint16_t ) ( ( float ) HIGH * breath_approx( led_pos ) );
    *led_duty =
        linear( *led_duty, target_duty, SPEED - led_array->time_counter );
}
...
uint64_t breathing( led_array_t* led_array ) {
    ...
    uint16_t diff =
        ( ( ( uint16_t ) ( ( ( float ) duty ) / 2000.0 ) ) << 1 ) + 1;

    bool* increasing = &led_array->increasing;

    *increasing = *increasing ^ ( duty > HIGH || duty < LOW + 1 );
    duty = duty + diff * ( *increasing ) - diff * !( *increasing );
