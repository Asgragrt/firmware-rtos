...
bool keyboard_update_buffer( keyboard_t* kbd, uint8_t* buffer,
    ...
    /*
    uint8_t offset = 0;
    bool center_keys_simul = !gpio_get(kbd_get_pin(kbd, PIN_COUNT - 1)) &&
    !gpio_get(kbd_get_pin(kbd, PIN_COUNT - 2)); if ( center_keys_simul )
    kbd->start_counter = true; if ( kbd->start_counter ){ kbd->counter++; if (
    kbd->counter >= 1500 ){ kbd->alternate_key ^= true; kbd->counter = 0;
    ...
    for (uint8_t i = 0; i < limit; i++){
        if( !gpio_get(kbd_get_pin(kbd, i)) ){
            ...
            memcpy(buffer + offset, kbd_get_keys(kbd, i), kbd_get_key_count(kbd,
    i)); new_status |= 1 << i; offset += kbd->pins[i].key_count;
        }
    }
...
// Flipped keys 05 00 69 09 00 0F FF 01 0E FF 02 0D FF 03 3A FF 04 12 E0 FF 05
// 2C FF 06 09 FF 07 07 FF 08 16 FF
//                 ^^
...
bool keyboard_update_key( keyboard_t* kbd ) {
    ...
    for ( uint8_t i = 0; i < key_modification_count; i++ ) {
        ...
        for ( uint8_t j = current_idx + 1;; j++ ) {
            ...
            kbd->pins[key_pin].keys[j - current_idx - 1] =
                flash_target_contents[j];
        }
    }
